...
fun DraggableContainer(
    ...
    BoxWithConstraints(modifier) {
        ...
        children.forEach { child ->
            ...
            val isSelected = when (textBoxInteractionState) {
                is TextBoxInteractionState.Selected ->
                    textBoxInteractionState.textBoxId == child.id
                is TextBoxInteractionState.Editing -> false
                TextBoxInteractionState.None -> false
            }

            val transformableState = rememberTransformableState { scaleChange, panChange, rotationChange ->
                if (!isSelected) return@rememberTransformableState
                ...
                onChildTransformChanged(child.id, newOffset, newRotation, newScale)
            }
            ...
            Box(
                ...
                MemeTextBox(
                    ...
                    onDoubleClick = { onChildDoubleClick(child.id) },
                    onTextChange = { onChildTextChange(child.id, it) },
                    onDeleteClick = { onChildDeleteClick(child.id) },
                )
            }
        }
    }
